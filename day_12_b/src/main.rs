...
fn nr_possible_arrangements(record: &str) -> u64 {
    ...
    let nr_arrangements = nr_assignments(0, &symbols, &counts, false);
    println!("Found {} arrangements", nr_arrangements);
    nr_arrangements
}

fn nr_assignments(level: usize, symbols: &String, counts: &[u32], in_hashes: bool) -> u64 {
    ...
    let result = if first_symbol == '#' {
        ...
        if in_hashes {
            ...
            if first_count == 0 {
                ...
                new_counts[0] -= 1;
                nr_assignments(level + 1, &rest_symbols, &new_counts, true)
            }
        } else {
        ...
        if in_hashes {
            ...
            if first_count == 0 {
                ...
                nr_assignments(level + 1, &rest_symbols, new_counts, false)
            } else {
            ...
            nr_assignments(level + 1, &rest_symbols, counts, false)
        }
    } else if first_symbol == '?' {
        let rest_symbols = String::from(&symbols[1..]);
        let dash_symbol = format!("#{}", &rest_symbols);
        let dot_symbol = format!(".{}", &rest_symbols);
        let hash_arrangements = nr_assignments(level + 1, &dash_symbol, counts, in_hashes);
        let dot_arrangements = nr_assignments(level + 1, &dot_symbol, counts, in_hashes);
